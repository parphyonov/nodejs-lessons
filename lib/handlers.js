        ...
        _data.read('users', phone, (err, data) => {
          if (!err && data) {
            _data.delete('users', phone, err => {
              if (!err) {
                const userChecks = data.checks.length > 0 ? data.checks : false;
                const checksToDelete = userChecks.length;
                if (checksToDelete > 0) {
                  let checksDeleted = 0;
                  let deletionErrors = false;
                  userChecks.forEach(userCheckId => {
                    _data.delete('checks', userCheckId, err => {
                      if (err) {
                        deletionErrors = true;
                      };
                      checksDeleted += 1;
                      if (checksDeleted === checksToDelete) {
                        if (!deletionErrors) {
                          ...
                          callback(500, {'error': 'Deleting checks'});
                        };
                      };
                    });
                  });
...
handlers._checks.delete = (data, callback) => {
  ...
  if (checkId) {
    _data.read('checks', checkId, (err, checkData) => {
      if (!err && checkData) {
        const tokenId = typeof(data.headers.token) === 'string' && data.headers.token.trim().length === 20 ? data.headers.token.trim() : false;
        handlers._tokens.verifyToken(tokenId, checkData.phone, tokenIsValid => {
          if (tokenIsValid) {
                      ...
                      if (checkIndex >= 0) {
                        ...
                        _data.update('users', checkData.phone, userData, err => {
                          if (!err) {
                            ...
                            callback(500, {error: 'Failed to update new user data!'});
                          };
                        });
                      } else {
                        callback(400, {error: 'The specified check does not belong to this user!'});
                      };
                    } else {
                      callback(400, {error: 'This user does not have any active checks yet!'});
                    };
                  } else {
                    callback(400, {error: 'User not found'});
                  };
        ...
        callback(404, {error: 'The check is not found!'})
      };
    });
  } else {
    callback(400, {error: 'Missing required field'});
